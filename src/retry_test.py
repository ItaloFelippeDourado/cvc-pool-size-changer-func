...
def handler(ctx, data: io.BytesIO=None):
    ...
    signer = oci.config.from_file(file_location="~/.oci/config", profile_name="DEFAULT")
    ...
    new_pool_size = calculateNewPoolSize(total_requests, pool_size, qtd_request_per_vm, asg_max_size)
...
def getCompartmentId(signer, pool_ocid):
    # Get Compartment_ID --------------------------------------------------------
    compute_management_client = oci.core.ComputeManagementClient(config=signer)
    try:
...
def getCurrentPoolSize(pool_ocid, signer):
    compute_client = oci.core.ComputeManagementClient(config=signer)
    try:
...
def getLoadBalancerRequestCount(backendSet, compartment_id, signer):
    monitoring_client = oci.monitoring.MonitoringClient(config=signer)
    try:
...
def calculateNewPoolSize(qtd_requests, pool_size, qtd_request_per_vm, asg_max_size):
